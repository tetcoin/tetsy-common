...
pub trait KeyValueDB: Sync + Send + tetsy_util_mem::MallocSizeOf {
	...
	/// Query statistics.
	///
	/// Not all tetsy-kvdb implementations are able or expected to implement this, so by
	/// default, empty statistics is returned. Also, not all tetsy-kvdb implementation
